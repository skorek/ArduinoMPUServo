...
void Get_Angles(long time)
{
    double dt = time / 1000.0 * 3.0; 
    GYRO_XOUT = GYRO_XOUT - GYRO_XOUT_OFFSET;
    GYRO_YOUT = GYRO_YOUT - GYRO_YOUT_OFFSET;
    GYRO_ZOUT = GYRO_ZOUT - GYRO_ZOUT_OFFSET;
    GYRO_XRATE = (float)GYRO_XOUT/gyro_sensitivity/3.0;
    GYRO_YRATE = (float)GYRO_YOUT/gyro_sensitivity/3.0;
    GYRO_ZRATE = (float)GYRO_ZOUT/gyro_sensitivity/3.0;
    ACCEL_XANGLE = 57.295*atan((float)ACCEL_YOUT/ sqrt(pow((float)ACCEL_ZOUT,2)+pow((float)ACCEL_XOUT,2)+0.01));
    ACCEL_YANGLE = 57.295*atan((float)ACCEL_XOUT/ sqrt(pow((float)ACCEL_ZOUT,2)+pow((float)ACCEL_YOUT,2)+0.01));
    GYRO_XANGLE += (GYRO_XRATE*dt);//*2.1);
    GYRO_YANGLE -= (GYRO_YRATE*dt);//*2.1);
    GYRO_ZANGLE += (GYRO_ZRATE*dt);//*2.1); 
...
void Get_Values()
{
  ...
  ACCEL_XOUT = accel_t_gyro.value.x_accel;
  ACCEL_YOUT = accel_t_gyro.value.y_accel;
  ACCEL_ZOUT = accel_t_gyro.value.z_accel;
  GYRO_XOUT  = accel_t_gyro.value.x_gyro;
  GYRO_YOUT  = accel_t_gyro.value.y_gyro;
  GYRO_ZOUT  = accel_t_gyro.value.z_gyro;
