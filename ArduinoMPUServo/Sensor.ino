...
//  float accx = RadToDeg*atan((float)accx_temp/ sqrt(pow((float)accz_temp,2)+pow((float)accy_temp,2)+0.01));
//  float accy = RadToDeg*atan((float)accy_temp/ sqrt(pow((float)accz_temp,2)+pow((float)accx_temp,2)+0.01));
  float accx = atan2(accx_temp,sqrt(pow((float)accz_temp,2)+pow((float)accy_temp,2)))*RadToDeg;
...
void updateAcc() { //High pass filter
  ...
  buffer[0] = buffer[0] - MPU.ACC_X_OFFSET;
  buffer[1] = buffer[1] - MPU.ACC_Y_OFFSET;
  buffer[2] = buffer[2] - MPU.ACC_Z_OFFSET;
#ifdef DEBUG_ACC
  Serial.print(buffer[0]);  Serial.print('\t');
  Serial.print(buffer[0]);  Serial.print('\t');
  Serial.print(buffer[1]);  Serial.print('\t');
  Serial.print(MPU.ACC_X_OFFSET); Serial.print('\t');
  Serial.print(MPU.ACC_Y_OFFSET); Serial.print('\t');
  Serial.print(MPU.ACC_Z_OFFSET); Serial.print('\t');
  Serial.print('\n');
#endif
...
void updateGyro() {
  ...
  gx_temp[GYRO_MAF_NR - 1] = (float)(buffer[0] - MPU.GYRO_X_OFFSET);
  gy_temp[GYRO_MAF_NR - 1] = (float)(buffer[1] - MPU.GYRO_Y_OFFSET);
  gz_temp[GYRO_MAF_NR - 1] = (float)(buffer[2] - MPU.GYRO_Z_OFFSET);
  gyroMAF();
}
